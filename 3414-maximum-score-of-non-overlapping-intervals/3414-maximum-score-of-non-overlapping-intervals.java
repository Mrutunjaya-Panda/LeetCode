class Solution {
    ...
    boolean isLexSmaller(List<Integer> a, List<Integer> b){
        ...
        for(int i=0;i<len;i++){
            if(!a.get(i).equals(b.get(i))){
                return a.get(i) < b.get(i);//return the one which is less index
            }
        }
        return a.size() < b.size();
    }
    public Node solve(int i, int k){
        if(k==0 || i>=n) return new Node();

        if(dp[i][k].score != -1) return dp[i][k];

        int wt = intervals[i][2];
        int idx = intervals[i][3];//original idx.
        int j = nextIdx[i];//j in sorted interval array

        //skip current index i interval
    ...
    public int[] maximumWeight(List<List<Integer>> intervalsList) {
        this.n = intervalsList.size();
        intervals = new int[n][4];//store each interval with 4 info. {l,r,wt,originalIdx}
        for (int i = 0; i < n; i++) {
